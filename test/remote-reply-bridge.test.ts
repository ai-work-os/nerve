...
const MAC_PORT = 4924;
const HOME_PORT = 4925;
const macData = mkdtempSync(join(tmpdir(), "nerve-mac-bridge-"));
const homeData = mkdtempSync(join(tmpdir(), "nerve-home-bridge-"));
...
async function main(): Promise<void> {
  ...
  await waitFor(async () => {
    const history = await post(MAC_PORT, "/channel/history", { channelId: ch.channelId, limit: 20 });
