...
export class Bus {
    ...
    // --- Node operations ---
    registerNode(ws, name, capabilities, permissions) {
        return this.nodePool.registerWebSocket(ws, name, capabilities, permissions);
    }
    async spawnNode(adapter, name, cwd) {
    ...
    spawnNodeSync(adapter, name, cwd) {
    ...
    removeNodeFromChannel(channelId, nodeName) {
        ...
        if (nodeId) {
            const node = this.nodePool.get(nodeId);
            if (node)
                node.channels.delete(channelId);
        }
        this.broadcastToChannel(channelId, {
            jsonrpc: "2.0",
            method: "channel.nodeLeft",
    ...
    // --- Messaging ---
    postMessage(channelId, from, content) {
        const ch = this.channels.get(channelId);
        if (!ch)
            return null;
    ...
    postFromProcess(nodeName, content) {
    ...
    getHistory(channelId, limit, before) {
        return this.store.getMessages(channelId, limit, before);
    }
    // --- Internal ---
    buildSystemPrompt(agentName, channelId, members) {
    ...
    broadcastToChannel(channelId, notification) {
        const ch = this.channels.get(channelId);
        if (!ch)
            return;
        for (const [, nodeId] of ch.nodes) {
            const node = this.nodePool.get(nodeId);
            if (node && node.isProcess)
                continue; // process nodes only speak ACP, skip bus broadcasts
            if (node && node.transport.alive) {
                node.transport.send(notification);
            }
        }
    }
    handleNodeEvent(event, node, detail) {
        switch (event) {
