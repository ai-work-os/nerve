...
async function cmdStatus() {
    try {
        ...
        if (health.logFile)
            console.log(`Log: ${health.logFile}`);
...
async function cmdChannel(sub, args) {
    try {
        switch (sub) {
            case "list":
            case "ls": {
            ...
            case "close": {
                ...
                if (!channelId)
                    die("Usage: nerve channel close <channelId>");
            ...
            case "hist": {
                const channelId = args[0];
                if (!channelId)
                    die("Usage: nerve channel history <channelId> [--limit N]");
                let limit = 20;
            ...
            case "post": {
                ...
                if (!channelId || !content)
                    die("Usage: nerve channel post <channelId> <message> [--from NAME]");
...
async function cmdNode(sub, args) {
    try {
        switch (sub) {
            case "list":
            case "ls": {
            ...
            case "spawn": {
                ...
                if (!adapter)
                    die("Usage: nerve node spawn <adapter> [--name NAME] [--cwd DIR]");
                let name, cwd;
                for (let i = 1; i < args.length; i++) {
            ...
            case "join": {
                ...
                if (!nodeName || !channelId)
                    die("Usage: nerve node join <nodeName> <channelId>");
            ...
            case "leave": {
                ...
                const channelId = args[1];
                if (!nodeName || !channelId)
                    die("Usage: nerve node leave <nodeName> <channelId>");
            ...
            case "stop": {
                const target = args[0];
                if (!target)
                    die("Usage: nerve node stop <nodeId|nodeName>");
...
if (!cmd || cmd === "--help" || cmd === "-h") {
    ...
    const sub = argv[1];
    if (!sub)
        die("Usage: nerve channel <list|create|close|history|post>");
    cmdChannel(sub, argv.slice(2));
}
else if (cmd === "node" || cmd === "n") {
    const sub = argv[1];
    if (!sub)
        die("Usage: nerve node <list|spawn|stop>");
