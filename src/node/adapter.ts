...
const adapters: Record<string, AdapterConfig> = {
  ...
  "screenshot": {
    type: "program",
    cmd: "npx",
    args: ["tsx", "src/plugins/screenshot/index.ts"],
    capabilities: ["monitor"],
    terminal: false,
    description: "手机截图接收桥：内容寻址存储 + 感知日志 + #screenshots 频道通知",
    commands: {
      status: { description: "查看状态：HTTP 端口、累计截图数、未投递 Mac 数" },
    },
    usage: [
      ...
      "  POST /screenshot/ack-mac     { blobId } — 标记已投递",
      "",
      "随 nerve 自启 (cli.ts startScreenshot)，--no-screenshot 跳过。",
    ].join("\n"),
  },
  "user-recorder": {
    type: "program",
